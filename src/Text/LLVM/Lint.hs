...
checkDefine d = enter (Function (defName d)) $ withRetType (defRetType d) $
  case defBody d of
    []      -> err "No basic blocks in function body"
    entry:_ -> do
      mapM_ (checkInputType . typedType) (defArgs d)
      checkOutputType (defRetType d)
...
checkInputType ty = when (containsVoid ty) $ err $ concat
  [ "Type ``", render (ppType ty), "'' is not a valid argument type"]

checkOutputType :: Type -> Lint ()
checkOutputType ty
  | ty `primTypeOf` isVoid = return ()
  | otherwise              = when (containsVoid ty) $ err $ concat
    [ "Type ``", render (ppType ty), "'' is not a valid return type"]
...
isVoid :: PrimType -> Bool
isVoid Void = True
isVoid _    = False

containsVoid :: Type -> Bool
containsVoid ty = ty `primTypeOf` isVoid || ty `pointsTo` containsVoid

pointsTo :: Type -> (Type -> Bool) -> Bool
pointsTo (PtrTo t) p = p t
pointsTo _         _ = False
